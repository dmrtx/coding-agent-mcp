...
3. **Phase 1 T3-backed creation and resume is worktree-only**:
   ...
   - Operations that resume or advance agent execution (`t3_continue_task`, `t3_respond_approval`, `t3_respond_user_input`) require a worktree-backed thread (`thread.worktreePath != null`) and a writable repository. A non-null `worktreePath`, not merely a branch name, is the evidence used by `coding-agent-mcp` to permit operations that advance execution. A branch alone is source-control metadata and is not sufficient evidence that execution is isolated from the configured repository root.
